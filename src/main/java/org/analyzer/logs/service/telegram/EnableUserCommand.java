...
import static org.analyzer.logs.service.telegram.EnableUserCommand.COMMAND_NAME;

@Component(COMMAND_NAME)
public class EnableUserCommand extends BotCommand {
    ...
    public EnableUserCommand() {
        super(COMMAND_NAME, "Enabling of user account in application");
    }
    ...
    public void execute(AbsSender absSender, User user, Chat chat, String[] arguments) {
        ...
        try {
            this.userService.findByTelegramId(user.getId())
                    .ifPresentOrElse(
                            userEntity -> {
                                this.userService.enable(userEntity.getUsername());
                                msg.setText("<b>Account enabled successfully.</b>");
                            },
                            () -> msg.setText("<b>Account not found to activation.</b>")
                    );
        } catch (UserNotDisabledException ex) {
            msg.setText("<b>User already enabled.</b>");
        }
