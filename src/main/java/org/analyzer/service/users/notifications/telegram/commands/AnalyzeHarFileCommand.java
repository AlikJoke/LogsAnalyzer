...
public class AnalyzeHarFileCommand extends BaseUploadingFileBotCommand implements TelegramCommandConversationChain {
    ...
    protected Optional<SendMessage> executeCommand(AbsSender absSender, User user, Chat chat, UserEntity userContext, String[] arguments) {
        ...
        final var msg = createReplyMessage(chat.getId(), ("Pass keys to additional filter requests from HAR " +
                "(to stop passing the filter, pass a %s):").formatted(SKIP_STAGE_STR_FORMATTED));
    ...
    public Optional<SendMessage> onMessageReceived(
        ...
        return switch (context.getLastStage()) {
            case FILTERS_STAGE -> {
                ...
                final var msgText = "Upload HAR file or pass already uploaded HAR key:";
